...
def run_pure_pursuit(iteration: int,
    ...
    with open(f'./MAP0/GP/Data/gp_iter{gp_iter-1}_{state_names[1]}.pickle', 'rb') as f:
            (vymodel, vyxscaler, vyyscaler, gpmodel) = pickle.load(f)
        
    with open(f'./MAP0/GP/Data/gp_iter{gp_iter-1}_{state_names[2]}.pickle', 'rb') as f:
    ...
    np.savez(
        os.path.join(out_dir_data, f'traj_info_{iteration+1}.npz'),
        x_ey=[], x_v=[], x_curv=[],
        x_true=xs_true, x_pred=np.zeros_like(xs_true),
        u=us
    )


    np.savez(os.path.join(out_dir_mpc, f'traj_iter{iteration+1}_{label}.npz'),
