...
for i in range(1,10):
    ...
    plannedtime = SolveMinimumLapTime(dir = './MAP0', model = 'DynGP', iter = i)
    traj, _, _, lap_time,col = MPCmain_forces(i, gp_train=True,  file_name = 'optimized_gp_traj', map_dir = f"./MAP0",  file_dir='/GP/', return_ = True, plot_ = False, label =  f'MAP{0}_DynGP')
    lap_time_list_gp.append(lap_time)
    planned_time_list.append(plannedtime)
