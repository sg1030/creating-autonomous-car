...
#include "../include/FORCES_MPC_GP_MAP0_DynGP_4_memory.h"
...
void copyCArrayToM_FORCES_MPC_GP_MAP0_DynGP_4_float(FORCES_MPC_GP_MAP0_DynGP_4_float *src, double *dest, solver_int32_default dim) 
{
    solver_int32_default i;
    for( i = 0; i < dim; i++ ) 
    {
...
void copyMArrayToC_FORCES_MPC_GP_MAP0_DynGP_4_float(double *src, FORCES_MPC_GP_MAP0_DynGP_4_float *dest, solver_int32_default dim) 
{
    solver_int32_default i;
    for( i = 0; i < dim; i++ ) 
    {
        *dest++ = (FORCES_MPC_GP_MAP0_DynGP_4_float) (*src++) ;
    }
}

void copyMValueToC_FORCES_MPC_GP_MAP0_DynGP_4_float(double * src, FORCES_MPC_GP_MAP0_DynGP_4_float * dest)
{
	*dest = (FORCES_MPC_GP_MAP0_DynGP_4_float) *src;
}

extern solver_int32_default (FORCES_MPC_GP_MAP0_DynGP_4_float *x, FORCES_MPC_GP_MAP0_DynGP_4_float *y, FORCES_MPC_GP_MAP0_DynGP_4_float *l, FORCES_MPC_GP_MAP0_DynGP_4_float *p, FORCES_MPC_GP_MAP0_DynGP_4_float *f, FORCES_MPC_GP_MAP0_DynGP_4_float *nabla_f, FORCES_MPC_GP_MAP0_DynGP_4_float *c, FORCES_MPC_GP_MAP0_DynGP_4_float *nabla_c, FORCES_MPC_GP_MAP0_DynGP_4_float *h, FORCES_MPC_GP_MAP0_DynGP_4_float *nabla_h, FORCES_MPC_GP_MAP0_DynGP_4_float *hess, solver_int32_default stage, solver_int32_default iteration, solver_int32_default threadID);
FORCES_MPC_GP_MAP0_DynGP_4_extfunc pt2function_FORCES_MPC_GP_MAP0_DynGP_4 = &;


/* Some memory for MEX function */
static FORCES_MPC_GP_MAP0_DynGP_4_params params;
static FORCES_MPC_GP_MAP0_DynGP_4_output output;
static FORCES_MPC_GP_MAP0_DynGP_4_info info;
static FORCES_MPC_GP_MAP0_DynGP_4_mem * mem;
...
void mexFunction( solver_int32_default nlhs, mxArray *plhs[], solver_int32_default nrhs, const mxArray *prhs[] )  
{
    ...
    if (nrhs != 1)
	{
		mexErrMsgTxt("This function requires exactly 1 input: PARAMS struct.\nType 'help FORCES_MPC_GP_MAP0_DynGP_4_mex' for details.");
	}    
	if (nlhs > 3) 
	{
    ...
    if (mem == NULL)
    {
        mem = FORCES_MPC_GP_MAP0_DynGP_4_internal_mem(0);
    }
	...
	#if SET_PRINTLEVEL_FORCES_MPC_GP_MAP0_DynGP_4 > 0
	...
	exitflag = FORCES_MPC_GP_MAP0_DynGP_4_solve(&params, &output, &info, mem, fp, pt2function_FORCES_MPC_GP_MAP0_DynGP_4);
	...
	*mxGetPr(plhs[1]) = (double)exitflag;
	...
	plhs[2] = mxCreateStructMatrix(1, 1, 20, infofields);
		...
		outvar = mxCreateDoubleMatrix(1, 1, mxREAL);
		copyCArrayToM_FORCES_MPC_GP_MAP0_DynGP_4_float((&(info.res_eq)), mxGetPr(outvar), 1);
		mxSetField(plhs[2], 0, "res_eq", outvar);


		/* scalar: inf-norm of inequality constraint residuals */
		outvar = mxCreateDoubleMatrix(1, 1, mxREAL);
		copyCArrayToM_FORCES_MPC_GP_MAP0_DynGP_4_float((&(info.res_ineq)), mxGetPr(outvar), 1);
		mxSetField(plhs[2], 0, "res_ineq", outvar);


		/* scalar: norm of stationarity condition */
		outvar = mxCreateDoubleMatrix(1, 1, mxREAL);
		copyCArrayToM_FORCES_MPC_GP_MAP0_DynGP_4_float((&(info.rsnorm)), mxGetPr(outvar), 1);
		mxSetField(plhs[2], 0, "rsnorm", outvar);


		/* scalar: max of all complementarity violations */
		outvar = mxCreateDoubleMatrix(1, 1, mxREAL);
		copyCArrayToM_FORCES_MPC_GP_MAP0_DynGP_4_float((&(info.rcompnorm)), mxGetPr(outvar), 1);
		mxSetField(plhs[2], 0, "rcompnorm", outvar);


		/* scalar: primal objective */
		outvar = mxCreateDoubleMatrix(1, 1, mxREAL);
		copyCArrayToM_FORCES_MPC_GP_MAP0_DynGP_4_float((&(info.pobj)), mxGetPr(outvar), 1);
		mxSetField(plhs[2], 0, "pobj", outvar);


		/* scalar: dual objective */
		outvar = mxCreateDoubleMatrix(1, 1, mxREAL);
		copyCArrayToM_FORCES_MPC_GP_MAP0_DynGP_4_float((&(info.dobj)), mxGetPr(outvar), 1);
		mxSetField(plhs[2], 0, "dobj", outvar);


		/* scalar: duality gap := pobj - dobj */
		outvar = mxCreateDoubleMatrix(1, 1, mxREAL);
		copyCArrayToM_FORCES_MPC_GP_MAP0_DynGP_4_float((&(info.dgap)), mxGetPr(outvar), 1);
		mxSetField(plhs[2], 0, "dgap", outvar);


		/* scalar: relative duality gap := |dgap / pobj | */
		outvar = mxCreateDoubleMatrix(1, 1, mxREAL);
		copyCArrayToM_FORCES_MPC_GP_MAP0_DynGP_4_float((&(info.rdgap)), mxGetPr(outvar), 1);
		mxSetField(plhs[2], 0, "rdgap", outvar);


		/* scalar: duality measure */
		outvar = mxCreateDoubleMatrix(1, 1, mxREAL);
		copyCArrayToM_FORCES_MPC_GP_MAP0_DynGP_4_float((&(info.mu)), mxGetPr(outvar), 1);
		mxSetField(plhs[2], 0, "mu", outvar);


		/* scalar: duality measure (after affine step) */
		outvar = mxCreateDoubleMatrix(1, 1, mxREAL);
		copyCArrayToM_FORCES_MPC_GP_MAP0_DynGP_4_float((&(info.mu_aff)), mxGetPr(outvar), 1);
		mxSetField(plhs[2], 0, "mu_aff", outvar);


		/* scalar: centering parameter */
		outvar = mxCreateDoubleMatrix(1, 1, mxREAL);
		copyCArrayToM_FORCES_MPC_GP_MAP0_DynGP_4_float((&(info.sigma)), mxGetPr(outvar), 1);
		...
		outvar = mxCreateDoubleMatrix(1, 1, mxREAL);
		copyCArrayToM_FORCES_MPC_GP_MAP0_DynGP_4_float((&(info.step_aff)), mxGetPr(outvar), 1);
		mxSetField(plhs[2], 0, "step_aff", outvar);


		/* scalar: step size (combined direction) */
		outvar = mxCreateDoubleMatrix(1, 1, mxREAL);
		copyCArrayToM_FORCES_MPC_GP_MAP0_DynGP_4_float((&(info.step_cc)), mxGetPr(outvar), 1);
		mxSetField(plhs[2], 0, "step_cc", outvar);


		/* scalar: total solve time */
		outvar = mxCreateDoubleMatrix(1, 1, mxREAL);
		copyCArrayToM_FORCES_MPC_GP_MAP0_DynGP_4_float((&(info.solvetime)), mxGetPr(outvar), 1);
		mxSetField(plhs[2], 0, "solvetime", outvar);


		/* scalar: time spent in function evaluations */
		outvar = mxCreateDoubleMatrix(1, 1, mxREAL);
		copyCArrayToM_FORCES_MPC_GP_MAP0_DynGP_4_float((&(info.fevalstime)), mxGetPr(outvar), 1);
