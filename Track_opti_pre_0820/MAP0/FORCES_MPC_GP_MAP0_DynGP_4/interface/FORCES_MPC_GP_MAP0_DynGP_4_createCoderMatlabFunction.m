...
% 
% FORCES_MPC_GP_MAP0_DynGP_4_CREATECODERMATLABFUNCTION(MODELNAME, BLOCKNAME)
%
% This function generates a Simulink Model with Fixed-step solver type named MODELNAME to add the FORCES_MPC_GP_MAP0_DynGP_4 FORCESPRO solver as 
% a MATLAB Function Simulink Block named BLOCKNAME so that it can be used for Simulink simulation and 
% Simulink Code Generation.
%
% FORCES_MPC_GP_MAP0_DynGP_4_CREATECODERMATLABFUNCTION(MODELNAME, BLOCKNAME, USECOMPACTBLOCK)
% can be used to generate a MATLAB Function Simulink Block with compact inputs and outputs.
%
% FORCES_MPC_GP_MAP0_DynGP_4_CREATECODERMATLABFUNCTION(MODELNAME, BLOCKNAME, USECOMPACTBLOCK, USEEXISTINGMODEL)
% can be used if the selected Simulink Model already exists in order to add the Simulink Block to it.
%
% FORCES_MPC_GP_MAP0_DynGP_4_CREATECODERMATLABFUNCTION(MODELNAME, BLOCKNAME, USECOMPACTBLOCK, USEEXISTINGMODEL, OPENSIMULINKMODEL)
% can be used to select whether to open the simulink model.
function FORCES_MPC_GP_MAP0_DynGP_4_createCoderMatlabFunction(modelname, blockname, useCompactBlock, useExistingModel, openSimulinkModel)

    if nargin < 1 || isempty(modelname)
        modelname = 'FORCES_MPC_GP_MAP0_DynGP_4_model';
    end
    if ~isvarname(modelname)
        error('Modelname must be a valid variable name');
    end
    if nargin < 2 || isempty(blockname)
        blockname = 'FORCES_MPC_GP_MAP0_DynGP_4_block';
    end
    ...
    function_name = 'FORCES_MPC_GP_MAP0_DynGP_4_coderFunction.m';
    position = [170, 99, 650, 520];
    parameter_sizes = struct('xinit', '[6,1]',...
                             'x0', '[160,1]',...
                             'all_parameters', '[160,1]');
    if useCompactBlock
        function_name = 'FORCES_MPC_GP_MAP0_DynGP_4_coderFunctioncompact.m';
