...
% 
% [OUTPUTS] = FORCES_MPC_GP_MAP0_DynGP_8(INPUTS) solves an optimization problem where:
% Inputs:
...
function [x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13, x14, x15, x16, x17, x18, x19, x20] = FORCES_MPC_GP_MAP0_DynGP_8(xinit, x0, all_parameters)
    
    [output, ~, ~] = FORCES_MPC_GP_MAP0_DynGP_8Buildable.forcesCall(xinit, x0, all_parameters);
    x01 = output.x01;
    x02 = output.x02;
    x03 = output.x03;
    x04 = output.x04;
