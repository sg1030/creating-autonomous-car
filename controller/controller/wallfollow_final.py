...
# Modified wallfollow.py to communicate estop.py -> unnecessary part.
...
class WallFollowNode(Node):
    ...
    def __init__(self):
        ...
        self._ttc_active = False
        self._ttc_speed_cap = None
        ...
        self.create_subscription(Bool,      '/safety/ttc_active',    self._ttc_active_cb, 10)
        self.create_subscription(Float32,   '/safety/ttc_speed_cap', self._ttc_speed_cap_cb, 10)
    ...
    def _ttc_active_cb(self, msg):
        self._ttc_active = bool(msg.data)
        self._log_throttle('ttc_active', f'[ttc] active={self._ttc_active}', 0.5)

    def _ttc_speed_cap_cb(self, msg):
        self._ttc_speed_cap = None if msg.data < 0.0 else float(msg.data)
        cap_text = 'none' if self._ttc_speed_cap is None else f'{self._ttc_speed_cap:.3f}'
        self._log_throttle('ttc_cap', f'[ttc] speed_cap={cap_text}', 0.5)
    ...
    def _compute(self, dt):
        ...
        # b: right-perpendicular beam (-90 deg), a: front-right beam (-20 deg)
        b_angle = -math.pi / 2.0
        ...
        speed_scale = max(0.35, 1.0 - 0.4 * steer_ratio) # Changed to ratio 0.4
        speed = self.speed * speed_scale

        if self._ttc_speed_cap is not None:
            original_speed = speed
            speed = min(speed, self._ttc_speed_cap)
            if speed <= 1e-3 and self._ttc_active:
                self._integral_error = 0.0
                self._prev_error = error
            self._log_throttle(
                '[compute] ttc_cap',
                f'[compute] TTC speed cap applied: {original_speed:.3f}->{speed:.3f}',
                0.5,
            )
