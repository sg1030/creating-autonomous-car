...
class GapFollowNode(Node):
    ...
    def _publish_best_gap_debug(self, free_space, angles, gap_start, gap_end, target_idx, speed):
        if not self.debug_viz:
            return
        ...
        marker = self._make_marker(1, Marker.SPHERE, stamp, frame_id, ns='best_gap')
        ...
        line = self._make_marker(2, Marker.LINE_STRIP, stamp, frame_id, ns='best_gap')
        ...
        heading_deg = math.degrees(float(angles[target_idx]))
        text = self._make_marker(3, Marker.TEXT_VIEW_FACING, stamp, frame_id, ns='best_gap')
        ...
        text.text = f'heading={heading_deg:.1f} deg | speed={speed:.2f} m/s'
        markers.append(text)
    ...
    def _compute(self):
        ...
        if not np.any(valid_mask):
            empty_space = np.zeros_like(ranges)
            self._publish_all_gap_markers(empty_space, angles, [])
            self._publish_best_gap_debug(empty_space, angles, None, None, None, 0.0)
            return 0.0, 0.0
        ...
        free_space[max(0, closest_idx - bubble_size):min(len(free_space), closest_idx + bubble_size + 1)] = 0.0
        ...
        if gap_start is None or gap_end is None:
            self._publish_all_gap_markers(gap_free_space, angles, candidate_gaps)
            self._publish_best_gap_debug(gap_free_space, angles, None, None, None, 0.0)
            return 0.0, 0.0
        ...
        self._publish_all_gap_markers(gap_free_space, angles, candidate_gaps)
        self._publish_best_gap_debug(gap_free_space, angles, gap_start, gap_end, best_idx, speed)
        return steer, speed
